...
import {
  MapPin, Clock, Calendar, Users, IndianRupee, Check,
...
export default function DashboardActivityBookingPage() {
  const { id } = useParams();
  ...
  async function handleBookTicket(e) {
    ...
    if (!customerName || !customerEmail || !customerPhone || !travelDate) {
      setBookingError("Please fill in customer details and select a travel date.");
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 max-w-sm mx-auto">{error || "The requested activity is unavailable."}</p>
          <Link href="/dashboard/activities" className="inline-block px-6 py-2.5 bg-indigo-600 text-white rounded-xl font-bold text-sm">
            Back to Activities
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans pb-24">
      ...
      <div className="bg-white border-b border-slate-200/80 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link href="/dashboard/activities" className="flex items-center gap-2 text-slate-600 hover:text-slate-900 text-sm font-semibold">
            <ArrowLeft className="w-4 h-4" /> Back to Activities
          </Link>
          <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-50 text-rose-700 text-xs font-extrabold border border-rose-200">
            <Ticket className="w-3.5 h-3.5 text-rose-600" /> Direct Ticket Booking
          </span>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          {/* Activity Info (7 cols) */}
          <div className="lg:col-span-7 space-y-6">
            <div className="relative rounded-3xl overflow-hidden bg-slate-900 h-64 sm:h-80 border border-slate-200 shadow-lg">
              {coverImage ? (
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  {activity.duration && (
                    <span className="flex items-center gap-1 text-xs text-slate-200 bg-slate-900/60 backdrop-blur-md px-3 py-1 rounded-full border border-slate-700">
                      <Clock className="w-3.5 h-3.5 text-indigo-300" /> {activity.duration}
                    </span>
                  )}
                </div>
                ...
                {activity.city && (
                  <p className="text-sm text-indigo-200 font-semibold mt-1 flex items-center gap-1.5">
            ...
            <div className="bg-white rounded-3xl border border-slate-200/90 p-6 space-y-3 shadow-xs">
              <h2 className="text-base font-black text-slate-900 flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-indigo-600" /> About this Activity
              </h2>
              <p className="text-slate-600 text-sm leading-relaxed">
                {activity.description || "Experience top-rated sightseeing and adventure with guided booking."}
              </p>
            </div>
            ...
            {activity.inclusions?.length > 0 && (
              <div className="bg-white rounded-3xl border border-slate-200/90 p-6 space-y-3 shadow-xs">
                <h2 className="text-base font-black text-slate-900 flex items-center gap-2">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600" /> Ticket Inclusions
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {activity.inclusions.map((inc, i) => (
                    <div key={i} className="flex items-center gap-2 p-3 rounded-2xl bg-emerald-50 border border-emerald-100 text-emerald-900 text-xs font-bold">
          ...
          {/* Booking Panel (5 cols) */}
          <div className="lg:col-span-5 sticky top-24">
            <div className="bg-white rounded-3xl border border-slate-200/90 shadow-xl p-6 space-y-5">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div>
                  <span className="text-xs text-slate-400 font-extrabold uppercase tracking-wider">Ticket Rate</span>
                ...
                {childRate > 0 && (
                  <div className="text-right">
                    <span className="text-xs text-slate-400 font-extrabold uppercase tracking-wider">Child Rate</span>
              ...
              <form onSubmit={handleBookTicket} className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-extrabold text-slate-800 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                    <Calendar className="w-3.5 h-3.5 text-indigo-600" /> Select Travel Date *
                  </label>
                  <input
                    type="date"
                    value={travelDate}
                    ...
                    required
                    className="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-slate-900 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600"
                  />
                </div>
                ...
                {activity.availableSlots?.length > 0 && (
                  <div>
                    <label className="block text-xs font-extrabold text-slate-800 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-indigo-600" /> Preferred Slot *
                    </label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Adults</label>
                    <input
                      type="number"
                      min={1}
                      max={20}
                      ...
                      className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-slate-900 text-sm font-bold text-center"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Children</label>
                    <input
                      type="number"
                      min={0}
                      max={20}
                ...
                <div className="pt-2 space-y-3 border-t border-slate-100">
                  ...
                  <input
                    ...
                    required
                    className="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-slate-900 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600"
                  />
                  <input
                    type="email"
                    value={customerEmail}
                ...
                {/* Total Price Banner */}
                <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 text-white shadow-md space-y-1">
                ...
                >
                  ...
                  {booking ? "Generating Ticket Pass…" : "Book Ticket Pass Now"}
                </button>
              </form>
            </div>
          </div>
